...
function assert(ctx, cb) {
    ...
    expected = parseString(expected.toString());
...
function root(ctx, cb) {
    var res = process.env['SHY_PROJ'];

    if (typeof res == 'undefined') {
        err('SHY_PROJ not defined, possibly a bug')
        cb(1);
        return;
    }
